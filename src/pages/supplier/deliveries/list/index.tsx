...
const getDateTitle = (type: string) => {
  if (type === 'today') {
    ...
    const timestamp = (new Date()).getTime() + 2 * 24 * 60 * 60 * 1000;
...
const defaultColumns: GridColDef[] = [
  ...
  },
  {
    ...
    minWidth: 120,
    field: 'today',
    headerName: getDateTitle('today'),
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>-</Typography>
  },
  {
    flex: 0.2,
    minWidth: 120,
    field: 'tomorrow',
    headerName: getDateTitle('tomorrow'),
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>-</Typography>
  },
  {
    flex: 0.2,
    minWidth: 120,
    field: 'afterTomorrow',
    headerName: getDateTitle('afterTomorrow'),
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>-</Typography>
  }
]
  ...
  return (
    <DatePickerWrapper>
      <Grid container spacing={6}>
        ...
        <Grid item xs={12}>
          <Card>
            <TableHeader value={value} selectedRows={selectedRows} handleFilter={handleFilter} handleSetUpOrder={toggleAddCustomerDrawer} />
            <DataGrid
              autoHeight
              pagination
              rowHeight={62}
