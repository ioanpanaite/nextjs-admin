...
const DialogEditCustomer = (props: Props) => {
  // hook
  ...
  useEffect(() => {
    const setValues = {
      ...
      siteName: user?.siteName as string || '',
      accountId: user?.accountId as string || '',
      deliveryAddress: user?.deliveryAddress as string || '',
      phone: user?.contact as string || '',
      email: user?.email as string || '',
      name: user?.username as string || '',
      imgSrc: user?.avatarImage as string || '',
    }
    setImgSrc(user?.avatarImage as string || '');
    reset(setValues)
  }, [user, reset])

  const onSubmit = async (data: UserData) => {
    const result = await axios.post('/api/customers/update-customer', { data: { ...data, id: user?._id || '' } })
