...
const SidebarLeft = (props: ChatSidebarLeftType) => {
  ...
  const handleChatClick = async (type: 'chat' | 'contact', id: string | number) => {
    ...
    if (type === 'chat') {
      try {
        ...
        const selectedRoomId = store.selectedChat?.chat.id
        if (room && selectedRoomId !== room.roomId) {
          leaveChat()
      ...
      try {
        const chat = store.selectedChat?.chat
        if (id !== chat?.userId) {
          leaveChat()
          useChat({ roomId: chat?.id || id, userId: id })
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (store && store.contacts) {
      if (active !== null) {
        if (active.type === 'contact' && active.id) {
        ...
        return arrToMap.map((chat: ChatsArrType, index: number) => {
          ...
          return (
            <ListItem key={index} disablePadding sx={{ '&:not(:last-child)': { mb: 1 } }}>
              <ListItemButton
                disableRipple
                onClick={() => handleChatClick('chat', chat.chat.id)}
                sx={{
                  py: 2,
                  px: 3,
                  width: '100%',
              ...
              >
                <ListItemAvatar sx={{ m: 0, alignSelf: 'center' }}>
                  ...
                  >
                    {chat.image ? (
                      ...
                      >
                        {chat.fullName && getInitials(chat.fullName)}
                      </CustomAvatar>
                    )}
                  </Badge>
                </ListItemAvatar>
                ...
                >
                  ...
                  >
                    <>{lastMessage ? formatDateToMonthShort(lastMessage.time as string, true) : formatDateToMonthShort((new Date()).toLocaleString(), true)}</>
                  </Typography>
                ...
                >
                  <ListItemAvatar sx={{ m: 0 }}>
                    {contact.image ? (
                      ...
                      >
                        {contact.fullName && getInitials(contact.fullName)}
                      </CustomAvatar>
                    )}
                  </ListItemAvatar>
                  <ListItemText
                    ...
                    primary={<Typography variant='h6'>{contact.fullName ? contact.fullName : contact.email}</Typography>}
                    secondary={
                      <Typography noWrap sx={{ ...(!activeCondition && { color: 'text.secondary' }) }}>
                        {contact.about}
                      </Typography>
  ...
  const handleFilter = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (store.chats !== null && store.contacts !== null) {
      const searchFilterFunction = (contact: ChatsArrType | ContactType) =>
        contact.fullName && contact.fullName.toLowerCase().includes(e.target.value.toLowerCase())
