...
const ChatLog = (props: ChatLogType) => {
  // ** Props
  const { data, hidden, checkMsg, store } = props
        ...
        >
          <div>
            <CustomAvatar
              ...
              {...(data.contact.image && !isSender
                ? {
                  src: data.contact.image || undefined,
                  alt: data.contact.fullName
                }
                : {})}
              {...(isSender
                ? {
                  src: data.userContact.image || undefined,
            ...
            {item.messages.map((chat: ChatLogChatType, index: number, { length }: { length: number }) => {
              ...
              return (
                <Box key={index} sx={{ '&:not(:last-of-type)': { mb: 3 } }}>
                  <div style={{ display: 'flex', justifyContent: isSender ? 'end' : undefined, alignItems: 'center' }}>
                    {isSender && <i className="ri-check-double-line" style={{ fontSize: '20px', marginRight: '4px', color: chat.feedback.isSeen ? 'green' : undefined }}></i>}
                    <Typography
                      sx={{
                        boxShadow: 1,
                        borderRadius: 1,
